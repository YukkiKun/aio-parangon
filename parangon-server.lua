...
function parangon_addon.sendInformations(msg, player)
    ...
    temp.level = parangon.account[player:GetGUIDLow()].level
    temp.points = player:GetData('parangon_points')
    temp.exps = {
        exp = parangon.account[player:GetGUIDLow()].exp,
        exp_max = parangon.account[player:GetGUIDLow()].exp_max
    }
...
function parangon.onServerStart(event)
    ...
    CharDBExecute('CREATE TABLE IF NOT EXISTS `'..parangon.config.db_name..'`.`account_parangon` (`guid` INT(11) NOT NULL, `level` INT(11) DEFAULT 1, `exp` INT(11) DEFAULT 0, PRIMARY KEY (`guid`) );');
    CharDBExecute('CREATE TABLE IF NOT EXISTS `'..parangon.config.db_name..'`.`characters_parangon` (`guid` INT(11) NOT NULL, `strength` INT(11) DEFAULT 0, `agility` INT(11) DEFAULT 0, `stamina` INT(11) DEFAULT 0, `intellect` INT(11) DEFAULT 0, PRIMARY KEY (`guid`));');
...
function parangon.onLogin(event, player)
    ...
    if getParangonCharInfo then
        ...
        local pGuid = player:GetGUIDLow()
        CharDBExecute('INSERT INTO `'..parangon.config.db_name..'`.`characters_parangon` VALUES ('..pGuid..', 0, 0, 0, 0)')
    ...
    if not parangon.account[pAcc] then
        ...
        local getParangonAccInfo = AuthDBQuery('SELECT level, exp FROM `'..parangon.config.db_name..'`.`account_parangon` WHERE guid = '..pAcc)
...
function parangon.onLogout(event, player)
    ...
    CharDBExecute('REPLACE INTO `'..parangon.config.db_name..'`.`characters_parangon` VALUES ('..pGuid..', '..strength..', '..agility..', '..stamina..', '..intellect..')')
